...
def obtain_counts():
    ...
    unique_words = len(set(count_words()))
    for line in lines:
        if line.istitle():
            chapters += 1
...
def count_words(option = True):
    ...
    return sorted(list(word_counts.items()), key=lambda x: x[0])
